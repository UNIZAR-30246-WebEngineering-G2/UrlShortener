...
public class RequestBlockerServiceImpl extends CoordinatesHelper implements RequestBlockerService{

    @Autowired private ClickRepository clickRepository;
    @Autowired private IPServiceImpl ipServiceImpl;

    private static final Logger logger = LoggerFactory.getLogger(RequestBlockerServiceImpl.class);
    private final Integer SECONDS_FOR_REQUESTS = 10;

    public boolean tooMuchRequests(HttpServletRequest request, String hash){
        String ip = ipServiceImpl.extractIP(request);
